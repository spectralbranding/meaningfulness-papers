...
[internal path removed] §"The per-paper metadata bundle":
...
Module discovery: [internal path removed]*.yaml  +  research/**/ONTOLOGY.yaml
The `paper_key` declared INSIDE each module is authoritative (not the path).

Run:  uv run python code/build_ontology.py            # dry-run
      uv run python code/build_ontology.py --apply
      uv run python code/build_ontology.py --strict   # warnings -> errors
"""
...
def discover_modules() -> list[Path]:
    ...
    # foam-bridge-note.yaml). UPPERCASE-stem YAMLs in [internal path removed] are
...
def main() -> None:
    ...
    con.close()
    print(
        "Regenerate the .sql mirror: uv run python code/sync_substrate.py --sql-only"
    )
