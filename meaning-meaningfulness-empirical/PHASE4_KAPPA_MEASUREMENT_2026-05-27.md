---
title: "Phase 4 inter-coder κ measurement — protocol setup + recruitment status + execution-deferred-to-v1.4.0"
date: 2026-05-27
...
status: "Protocol documented + recruitment status surfaced + EXECUTION DEFERRED to v1.4.0 pending second-coder onboarding"
---
...
Grok r1 (postdraft) elevated κ measurement from a v1.2.0 Limitations item to a "must-fix before any submission" item (Priority-1 in §5 Major Revision Roadmap). The v1.2.0 release explicitly schedules κ measurement as Task δ; this document is the execution-readiness pass per Session H init prompt.
...
Two of the five extracted spines at v1.2.0:
...
Alternative (skip-OCR-correction path): proceed with the existing OCR'd extraction; report κ as "with OCR noise floor" lower-bound. Acceptable if Perplexity API access is unavailable or if v1.4.0 execution-time prioritizes the κ measurement over the OCR-correction.
...
The user operates as a solo researcher on this corpus. No second coder has been onboarded by Session H end. Per Session H init prompt: "If second coder unavailable: document the protocol setup + recruitment status; mark κ-measurement execution deferred to v1.4.0; surface as honest disclosure in Phase 5 paper.md revision".

**Per feedback_rank_twin_candidates_with_sourcing_options.md** (which extends to coder-recruitment): below are 5 ranked candidate-second-coder options for the user to choose from when v1.4.0 execution begins. Ranked by estimated κ-yield-quality × onboarding-cost.
...
| 4 | **Two AI-assisted coders (different model families: Claude + Gemini OR Claude + GPT-4o) acting in coordination with a human spot-check** | LOW-MEDIUM | LOW | Cheap to execute; produces a synthetic-κ that is methodologically informative but does NOT satisfy the "independent human coder" specification axiom A1 requires per Paper A. Acceptable as a v1.3.5 interim measurement only if explicitly framed as machine-coding-consistency, not as κ-on-axiom-A1. |
...
## Integration into v1.3.0 paper.md (Phase 5 5a)

The Phase 5 integrative-theory rewrite and the §Limitations / §Method-extraction-reliability subsection cite this protocol document as the pre-registered v1.4.0 execution plan. Honest disclosure language: "Inter-coder κ measurement on axiom A1 is pre-registered at v1.4.0 per `PHASE4_KAPPA_MEASUREMENT_2026-05-27.md`; the codebook (paper_a:appendix_A_schema), sampling frame (EM2000 focal + Grant 1996 KBV), and falsifier specification are locked alongside the v1.3.0 release. Axiom A1 reliability is currently asserted; the measurement will land before any tier-1 venue submission."

## Cross-reference

- Source: Session H init prompt §Phase 4 Task δ + SESSION_F_COMPLETION_2026-05-27.md §v1.2.0 task α/β/γ/δ — DEFERRED + Paper A 2026ao §Method-single-coder-reliability + paper_a:axiom_A1.
...
- Successor: PHASE4_KAPPA_RESULTS_v1.4.0.md (to be created at v1.4.0 execution).

---

*Phase 4 closes with full protocol documentation + recruitment status surfaced + execution deferred to v1.4.0. The Priority-1 must-fix item Grok r1 elevated remains pre-registered and tractable; what is missing is operator-attention-budget for second-coder onboarding, not framework-design clarity.*
